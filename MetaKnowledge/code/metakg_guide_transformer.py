...
class Transformer(Module):
    ...
    @staticmethod
    def generate_square_subsequent_mask(sz: int, device='cuda') -> Tensor:
...
class TransformerEncoder(Module):
    ...
    def forward(self, src: Tensor, metakg: Tensor, mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None) -> Tensor:
        ...
        for mod in self.layers:
            output = mod(output, metakg, src_mask=mask, src_key_padding_mask=src_key_padding_mask_for_layers)
...
class TransformerEncoderLayer(Module):
    ...
    def forward(self, src: Tensor, metakg: Tensor, src_mask: Optional[Tensor] = None,
        ...
        Shape:
            see the docs in Transformer class.
        """
        src2 = self.self_attn(src, src, src, metakg, attn_mask=src_mask,
...
class MultiheadAttention(Module):
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, metakg: Tensor, key_padding_mask: Optional[Tensor] = None,
    ...
    Shapes for outputs:
        ...
        if not self._qkv_same_embed_dim:
            ...
            return multi_head_attention_forward(
                query, key, value, metakg, self.embed_dim, self.num_heads,
                self.in_proj_weight, self.in_proj_bias,
                self.bias_k, self.bias_v, self.add_zero_attn,
                self.dropout, self.out_proj.weight, self.out_proj.bias,
                training=self.training,
...
def multi_head_attention_forward(
    ...
    metakg = metakg.unsqueeze(0)
    metakg = metakg.expand(bsz, -1, -1)

    q = q.contiguous().view(tgt_len, bsz * num_heads, head_dim).transpose(0, 1)
    metakg = metakg.contiguous().view(-1, bsz * num_heads, head_dim).transpose(0, 1)
    ...
    attn_output_weights_extra = torch.bmm(q, metakg.transpose(1, 2))
    # print("attn_output_weights_extra.shape=",attn_output_weights_extra.shape, "attn_output_weights.shape=", attn_output_weights.shape)
    attn_output_weights = torch.cat((attn_output_weights,attn_output_weights_extra), dim=-1)
    attn_output_weights = softmax(attn_output_weights, dim=-1)
    attn_output_weights = dropout(attn_output_weights, p=dropout_p, training=training)
    v = torch.cat((v, metakg), dim=1)
    attn_output = torch.bmm(attn_output_weights, v)
    # print("v.shape=", v.shape, "== attn_output_weights.shape=",attn_output_weights.shape, "attn_output.shape=", attn_output.shape,"==")
